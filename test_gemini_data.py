...
test_results = {}
success_ratios = {}
for index, data in gemini_outputs.items():
    try:
        local_env = {}
        code_blocks = re.findall(r"```python(.*?)```", data["result_prompts"] + data["result_tests"], re.DOTALL)
...
with open("gemini_test_results.txt", "w", encoding="utf-8") as file:
    for idx, out in test_results.items():
        file.write(f"--- Test Case {idx} ---\n{out}\n")

with open("gemini_success_ratios.json", "w", encoding="utf-8") as file:
